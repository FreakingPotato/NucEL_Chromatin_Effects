...
CKPT=f"{SWEEP}/ckpt_ntv2_lr{LR:g}"; os.makedirs(CKPT, exist_ok=True)
OUTJSON=f"{SWEEP}/ntv2_lr{LR:g}.json"
MODEL="InstaDeepAI/nucleotide-transformer-v2-100m-multi-species"
DEV="cuda"
MAXLEN=200          # matches single-config train_ntv2.py
BATCH=16
EPOCHS=8            # max epochs
...
log(f"=== NTv2 LR SWEEP RUN lr={LR:g} ===")
log("loading data")
df=pd.read_csv(DATA, sep="\t")
tr_full=df[df.split=="train"].reset_index(drop=True)
te=df[df.split=="test"].reset_index(drop=True)
# 10% stratified val slice, seed 42 -- IDENTICAL logic to train_nucel_lr.py for fairness
...
log("loading tokenizer + model (NTv2)")
tok=AutoTokenizer.from_pretrained(MODEL, trust_remote_code=True)
log("tokenizer model_max_length", tok.model_max_length)
model=AutoModelForSequenceClassification.from_pretrained(MODEL, num_labels=2, trust_remote_code=True).to(DEV)
...
def safe_auc(y,p):
    # returns (auroc, auprc, ok) ; guards against NaN/constant (divergence)
    if not np.isfinite(p).all():
        return 0.5, float(y.mean()), False
    try:
        return float(roc_auc_score(y,p)), float(average_precision_score(y,p)), True
    except Exception:
        return 0.5, float(y.mean()), False

curve=[]
best_auc=-1; best_step=-1; bad=0; gstep=0
stop=False; diverged=False
t0=time.time()
for ep in range(1, EPOCHS+1):
    model.train()
    win_loss=0.0; win_n=0
    for i,(bids, enc) in enumerate(tr_dl):
        enc={k:v.to(DEV) for k,v in enc.items()}
        with torch.autocast("cuda", dtype=torch.bfloat16):
            out=model(**enc)
            loss=out.loss
        ...
        opt.step(); sched.step()
        gstep+=1; lv=loss.item(); win_loss+=lv; win_n+=1
        if not math.isfinite(lv):
            diverged=True
        if (i+1)%eval_every==0 or (i+1)==steps_per:
            tl=win_loss/max(1,win_n)
            _,vp,vy=evaluate(va_dl)
            vauc,vap,ok=safe_auc(vy,vp)
            cur_lr=sched.get_last_lr()[0]
            curve.append({"global_step":gstep,"epoch":ep,"train_loss_running":float(tl),
                          "val_auROC":float(vauc),"val_auPRC":float(vap),"lr":float(cur_lr),"ok":bool(ok)})
            log(f"ep{ep} step {i+1}/{steps_per} gstep {gstep} train_loss(win) {tl:.4f} val_auROC {vauc:.4f} val_auPRC {vap:.4f} lr {cur_lr:.2e} ok={ok}")
            win_loss=0.0; win_n=0
            model.train()
            if ok and vauc>best_auc:
...
log(f"training done in {train_seconds:.1f}s, best_step {best_step} best_val_auROC {best_auc:.4f}")

# test eval using best-by-VAL checkpoint (fall back to current weights if none ever improved)
if best_step>0 and os.path.exists(f"{CKPT}/best.pt"):
    ck=torch.load(f"{CKPT}/best.pt", map_location=DEV)
    model.load_state_dict(ck["model"])
    log("loaded best.pt for test eval")
else:
    log("WARNING: no best checkpoint (never finite-improved); evaluating current weights")
    diverged=True
tids,tp,ty=evaluate(te_dl)
tauroc,tauprc,tok_=safe_auc(ty,tp)
log(f"TEST auROC {tauroc:.4f} auPRC {tauprc:.4f} ok={tok_}")

out={"model":"ntv2","lr":float(LR),"best_val_auROC":float(best_auc),"best_step":int(best_step),
     "test_auROC":float(tauroc),"test_auPRC":float(tauprc),
     "diverged":bool(diverged or not tok_),
     "steps_per_epoch":int(steps_per),"eval_every":int(eval_every),
     "train_seconds":float(train_seconds),"n_test":int(len(ty)),"n_test_pos":int(ty.sum()),
     "curve":curve}
